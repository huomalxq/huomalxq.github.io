...
<section class="resume-sheet">
  <header class="resume-top">
    <div class="resume-top__copy">
      ...
      <h1>林祥庆</h1>
      <p class="resume-tagline">计算机科学与技术硕士在读，目标方向为 LLM 系统优化、Agent / RAG 应用开发与模型训练实验。</p>
  ...
  <div class="resume-metric-grid">
    <article class="resume-metric">
      ...
      <p>推测解码方案在不同模型配置下取得端到端加速，并保持输出质量稳定。</p>
    </article>
    <article class="resume-metric">
      <span>Table Recall</span>
      <strong>61.4% -> 81.6%</strong>
      <p>通过补齐字段语义、业务样例和指标口径描述，提升问数场景表结构召回。</p>
    </article>
    <article class="resume-metric">
      <span>Response Time</span>
      <strong>50s -> 30s</strong>
      <p>基于节点耗时与端到端指标定位瓶颈，优化 Agent 工作流编排和代码执行流程。</p>
    </article>
  </div>
    ...
    <aside class="resume-rail">
      <article class="resume-sidecard">
        ...
        <ul class="resume-mini-list">
          <li>推测解码中的草稿策略、接受率分析与 KV Cache 复用</li>
          <li>自然语言问数场景下的 RAG 检索、Text2SQL 与 SQL 校验</li>
          <li>面向数学推理任务的 LoRA SFT 与 DPO / GRPO 偏好对齐</li>
        </ul>
      </article>
